...
vocab = get_vocab('model/train.json')
label_map = get_label_map('model/train.json')
train_dataset = Mydataset('model/train.json', vocab, label_map)
valid_dataset = Mydataset('model/dev.json', vocab, label_map)
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for text, label, seq_len, sentence in tqdm(valid_dataloader, desc='eval: '):
            text = text.to(device)
            seq_len = seq_len.to(device)
            batch_tag = model(text, seq_len, sentence)
            for k in batch_tag:
                vecw.append(k)
    return vecw